...
VolumeInfo   = namedtuple("VolumeInfo",   ["catalog_name", "schema_name",
                                            "name", "volume_type",
                                            "storage_location"])
TableInfo    = namedtuple("TableInfo",    ["catalog_name", "schema_name",
                                            "name", "table_type"])
ColumnInfo   = namedtuple("ColumnInfo",   ["name", "data_type",
                                            "nullable", "comment"])
GrantInfo    = namedtuple("GrantInfo",    ["principal", "privilege",
                                            "object_type", "object_key"])
TagInfo      = namedtuple("TagInfo",      ["tag_name", "tag_value"])
FunctionInfo = namedtuple("FunctionInfo", ["catalog_name", "schema_name",
                                            "name", "description"])
GroupInfo    = namedtuple("GroupInfo",    ["name", "members"])
DroppedTableInfo = namedtuple("DroppedTableInfo", ["catalog_name", "schema_name",
                                                    "name", "dropped_at"])
...
_PAT_CREATE_CAT    = re.compile(
    r"^CREATE\s+CATALOG\s+(?P<ine>IF\s+NOT\s+EXISTS\s+)?`?(?P<name>\w+)`?",
    _F)
...
_PAT_CREATE_VOL    = re.compile(
    ...
    r"(?:\s+LOCATION\s+'(?P<loc>[^']*)')?(?:\s+COMMENT\s+'(?P<comment>[^']*)')?", _F)

# DROP VOLUME [IF EXISTS] cat.sch.vol
_PAT_DROP_VOL      = re.compile(
    r"^DROP\s+VOLUME\s+(?P<ie>IF\s+EXISTS\s+)?`?(?P<cat>\w+)`?\.`?(?P<sch>\w+)`?\.`?(?P<vol>\w+)`?", _F)

# DESCRIBE VOLUME [EXTENDED] cat.sch.vol
_PAT_DESCRIBE_VOL  = re.compile(
    r"^DESCRIBE\s+VOLUME\s+(?:EXTENDED\s+)?`?(?P<cat>\w+)`?\.`?(?P<sch>\w+)`?\.`?(?P<vol>\w+)`?", _F)
...
_PAT_SET_TAGS      = re.compile(
    ...
    r"\s+(?P<ref>\S+)\s+SET\s+TAGS\s*\((?P<tags>[^)]+)\)", _F)

_PAT_UNSET_TAGS    = re.compile(
    r"^ALTER\s+(?:TABLE|VIEW|CATALOG|SCHEMA|DATABASE|VOLUME|FUNCTION|MATERIALIZED\s+VIEW)"
    r"\s+(?P<ref>\S+)\s+UNSET\s+TAGS\s*\((?P<keys>[^)]+)\)", _F)

_PAT_SHOW_TAGS     = re.compile(
    r"^SHOW\s+TAGS\s+ON\s+(?:\w+\s+)?(?P<ref>\S+)", _F)
...
_PAT_COMMENT_ON    = re.compile(
    ...
    r"\s+(?P<ref>\S+)\s+IS\s+'(?P<comment>[^']*)'", _F)
...
_PAT_SET_OWNER     = re.compile(
    ...
    r"\s+SET\s+OWNER\s+TO\s+(?P<owner>\S+)", _F)
...
_NOOP_PATTERNS = [
    ...
    (re.compile(r"^DESCRIBE\s+EXTERNAL\s+LOCATION\b", _F),"DESCRIBE EXTERNAL LOCATION"),
    ...
    (re.compile(r"^DESCRIBE\s+STORAGE\s+CREDENTIAL\b", _F),"DESCRIBE STORAGE CREDENTIAL"),
    ...
    (re.compile(r"^DESCRIBE\s+SERVICE\s+CREDENTIAL\b", _F),"DESCRIBE SERVICE CREDENTIAL"),
...
# USE CATALOG name
_PAT_USE_CATALOG = re.compile(
    r"^USE\s+CATALOG\s+`?(?P<name>\w+)`?", _F)
...
_PAT_CREATE_SCHEMA = re.compile(
    ...
    r"(?:\s+MANAGED\s+LOCATION\s+'(?P<location>[^']*)')?", _F)
...
_PAT_DROP_SCHEMA = re.compile(
    ...
    r"`?(?P<ref>[\w.`]+)`?"
    r"(?:\s+(?P<cascade>CASCADE|RESTRICT))?", _F)
...
# UNDROP TABLE name
_PAT_UNDROP = re.compile(
    r"^UNDROP\s+TABLE\s+`?(?P<ref>[\w.`]+)`?", _F)

# SHOW TABLES DROPPED [IN schema]
_PAT_SHOW_DROPPED = re.compile(
    r"^SHOW\s+TABLES\s+DROPPED"
    r"(?:\s+(?:IN|FROM)\s+`?(?P<ref>[\w.`]+)`?)?", _F)
...
_PAT_GRANT_ON_SHARE = re.compile(
    r"^GRANT\s+(?P<priv>[\w\s,]+?)\s+ON\s+SHARE\s+(?P<share>\S+)\s+TO\s+(?P<principal>.+)", _F)
_PAT_REVOKE_ON_SHARE = re.compile(
    r"^REVOKE\s+(?P<priv>[\w\s,]+?)\s+ON\s+SHARE\s+(?P<share>\S+)\s+FROM\s+(?P<principal>.+)", _F)
...
_PAT_ALTER_GROUP = re.compile(
    r"^ALTER\s+GROUP\s+`?(?P<name>\w+)`?\s+(?P<action>ADD|REMOVE)\s+(?:USER|MEMBER)\s+(?P<member>.+)", _F)
...
_PAT_ALTER_VOLUME = re.compile(
    r"^ALTER\s+VOLUME\s+`?(?P<ref>[\w.`]+)`?\s+(?P<action>SET\s+OWNER\s+TO|RENAME\s+TO|SET\s+TAGS|UNSET\s+TAGS)", _F)
...
class UnityCatalogShim:
    ...
    def sql(self, query: str, *args, **kwargs):
        ...
        if _PAT_CREATE_FOREIGN_CAT.match(q):
            print("[Unity] CREATE FOREIGN CATALOG — no-op localmente "
        ...
        if m:
            print(f"[Unity] ALTER CATALOG {m.group('name')} SET OWNER "
        ...
        if m:
            ...
            rows = [("name", name), ("catalog_type", "MANAGED"),
                    ("storage_location", path), ("comment", "")]
        ...
        if m:
            return self.create_volume(
                m.group("cat"), m.group("sch"), m.group("vol"),
        ...
        if m:
            ...
            rows = [(v.catalog_name, v.schema_name, v.name,
                     v.volume_type, v.storage_location) for v in vols]
            schema_str = ("catalog_name STRING, schema_name STRING, "
                          "name STRING, volume_type STRING, storage_location STRING")
            return (self._spark.createDataFrame(rows, schema_str)
                    if rows else
                    self._spark.createDataFrame([], schema_str))
        ...
        if m:
            return self.grant(m.group("priv").strip(), "SHARE",
                              m.group("share").strip(),
                              m.group("principal").strip())
        m = _PAT_REVOKE_ON_SHARE.match(q)
        if m:
            return self.revoke(m.group("priv").strip(), "SHARE",
                               m.group("share").strip(),
                               m.group("principal").strip())
        ...
        if m:
            ...
            rows = [(g.principal, g.privilege, g.object_type, g.object_key)
                    for g in grants]
            schema_str = ("principal STRING, privilege STRING, "
        ...
        if m:
            schema_str = ("principal STRING, privilege STRING, "
                          "object_type STRING, object_key STRING")
            rows = [(g["principal"], g["privilege"],
                     g["object_type"], g["object_key"])
        ...
        if m:
            self.set_tags(m.group("ref"),
                          {"_comment": m.group("comment")})
            print(f"[Unity] COMMENT ON {m.group('otype')} "
        ...
        if m:
            print(f"[Unity] ALTER {m.group('otype')} {m.group('ref')} "
        ...
        if m:
            ...
            return (self._spark.createDataFrame(rows, "catalog STRING")
                    if rows else
                    self._spark.createDataFrame([], "catalog STRING"))
        ...
        if m:
            ...
            comment = m.group("comment") or ""
            return self.create_schema(cat, sch, comment=comment,
                                       if_not_exists=bool(m.group("ine")))
        ...
        if m:
            ...
            if len(parts) == 2:
                ...
                cat, sch = _CURRENT_CATALOG, parts[0]
            return self.drop_schema(cat, sch,
                                     if_exists=bool(m.group("ie")),
                                     cascade=m.group("cascade") == "CASCADE"
                                             if m.group("cascade") else False)
        ...
        if m:
            ...
            return (self._spark.createDataFrame(rows, "databaseName STRING")
                    if rows else
                    self._spark.createDataFrame([], "databaseName STRING"))
        ...
        if m:
            ...
            try:
                ...
                return self.create_function(cat, sch, func, definition=q,
                                             if_not_exists=bool(m.group("ine")))
        ...
        if m:
            ...
            try:
                ...
                return self.drop_function(cat, sch, func,
                                           if_exists=bool(m.group("ie")))
        ...
        if m:
            try:
                ...
                rows = [(f.catalog_name, f.schema_name, f.name)
                        for f in funcs]
        ...
        if m:
            rows = [(d["catalog"], d["schema"], d["table"], d["dropped_at"])
                    for d in _DROPPED_TABLES]
            schema_str = ("catalog STRING, schema STRING, "
                          "tableName STRING, droppedAt STRING")
            return (self._spark.createDataFrame(rows, schema_str)
                    if rows else
                    self._spark.createDataFrame([], schema_str))
        ...
        if m:
            return self.create_group(m.group("name"),
                                      if_not_exists=bool(m.group("ine")))
        m = _PAT_DROP_GROUP.match(q)
        if m:
            return self.drop_group(m.group("name"),
                                    if_exists=bool(m.group("ie")))
        ...
        if m:
            ...
            return (self._spark.createDataFrame(rows, "name STRING")
                    if rows else
                    self._spark.createDataFrame([], "name STRING"))
        ...
        if m:
            print(f"[Unity] GET '{m.group('src')}' TO '{m.group('dst')}' (no-op locally)")
            return None
        m = _PAT_PUT_FILE.match(q)
        if m:
            print(f"[Unity] PUT '{m.group('src')}' INTO '{m.group('dst')}' (no-op locally)")
            return None
    ...
    def create_catalog(self, name: str, comment: str = "",
        ...
        if "://" not in wh:
            ...
            for sch in ("default", "bronze", "silver", "gold"):
                pathlib.Path(os.path.join(wh, sch)).mkdir(parents=True,
                                                           exist_ok=True)
        ...
        print(f"[Unity] Catálogo '{name}' creado → {wh}"
              + (f" (MANAGED LOCATION: {location})" if location else ""))
        return None

    def drop_catalog(self, name: str, if_exists: bool = False,
    ...
    def list_catalogs(self) -> List[CatalogInfo]:
        ...
        return [CatalogInfo(name=n, comment="")
                for n in sorted(self._catalogs)]
    ...
    def create_schema(self, catalog_name: str, schema_name: str,
                      comment: str = "", if_not_exists: bool = False,
                      properties: Optional[Dict] = None) -> None:
    ...
    def drop_schema(self, catalog_name: str, schema_name: str,
                    if_exists: bool = False, cascade: bool = False) -> None:
        ...
        if catalog_name == default_cat:
            self._spark.sql(
                f"DROP DATABASE {ie} `{schema_name}` {casc}".strip()
            )
        else:
            try:
                self._spark.sql(
    ...
    def list_schemas(self, catalog_name: Optional[str] = None) -> List[SchemaInfo]:
        ...
        try:
            ...
            return [SchemaInfo(catalog_name=cat, name=r[0], comment="")
                    for r in rows]
        except Exception:
            return []

    def describe_schema(self, catalog_name: str,
                        schema_name: str):
        ...
        try:
            ...
            rows = [("Database Name", schema_name),
    ...
    def list_tables(self, catalog_name: Optional[str] = None,
        ...
        try:
            if cat == default_cat:
                rows = self._spark.sql(
                    f"SHOW TABLES IN `{sch}`"
                ).collect()
            else:
                rows = self._spark.sql(
                    f"SHOW TABLES IN `{cat}`.`{sch}`"
                ).collect()
            return [TableInfo(catalog_name=cat, schema_name=sch,
                              name=r[1], table_type="MANAGED")
                    for r in rows]
        except Exception:
            return []
    ...
    def list_columns(self, catalog_name: str, schema_name: str,
                     table_name: str) -> List[ColumnInfo]:
        """Lista las columnas de una tabla."""
        df = self.describe_table(
    ...
    def create_volume(self, catalog_name: str, schema_name: str,
                      volume_name: str, volume_type: str = "MANAGED",
                      location: Optional[str] = None, comment: str = "",
    ...
    def drop_volume(self, catalog_name: str, schema_name: str,
                    volume_name: str, if_exists: bool = False) -> None:
        key = f"{catalog_name}.{schema_name}.{volume_name}"
    ...
    def list_volumes(self, catalog: Optional[str] = None,
                     schema: Optional[str] = None) -> List[VolumeInfo]:
        ...
        for key, path in self._volumes.items():
            ...
            results[key] = VolumeInfo(
                ...
                volume_type="MANAGED", storage_location=path,
            )
    ...
    def volume_path(self, catalog_name: str, schema_name: str,
                    volume_name: str, *subpath: str) -> str:
        """
    ...
    def set_tags(self, object_ref: str,
    ...
    def grant(self, privilege: str, object_type: str,
              object_ref: str, principal: str) -> None:
        ...
        print(f"[Unity] GRANT {privilege} ON {object_type} "
              f"{object_ref} TO {principal}")

    def revoke(self, privilege: str, object_type: str,
               object_ref: str, principal: str) -> None:
        """Revoke de un privilegio (no-op, registrado en audit log)."""
        self._grants[:] = [
            g for g in self._grants
            if not (g["principal"] == principal
                    and g["privilege"] == privilege
                    and g["object_key"] == object_ref)
        ]
        print(f"[Unity] REVOKE {privilege} ON {object_type} "
              f"{object_ref} FROM {principal}")

    def show_grants(self, principal: Optional[str] = None,
                    object_type: Optional[str] = None,
                    object_ref: Optional[str] = None) -> List[GrantInfo]:
        ...
        return [GrantInfo(principal=g["principal"],
    ...
    def audit_log(self):
        ...
        rows = [(g["principal"], g["privilege"], g["object_type"],
                 g["object_key"], g.get("granted_at", ""))
                for g in self._grants]
        schema_str = ("user_identity STRING, action_name STRING, "
    ...
    def create_row_filter(self, catalog_name: str, schema_name: str,
                          function_name: str, **kwargs) -> None:
        print(f"[Unity] create_row_filter '{function_name}' (no-op locally)")

    def create_column_mask(self, catalog_name: str, schema_name: str,
                           function_name: str, **kwargs) -> None:
    ...
    def create_external_location(self, name: str, url: str,
                                  credential: str, **kwargs) -> None:
    ...
    def create_function(self, catalog_name: str, schema_name: str,
                        function_name: str, definition: str = "",
                        description: str = "",
                        if_not_exists: bool = False) -> None:
    ...
    def drop_function(self, catalog_name: str, schema_name: str,
                      function_name: str,
    ...
    def list_functions(self, catalog_name: Optional[str] = None,
                       schema_name: Optional[str] = None) -> List[FunctionInfo]:
    ...
    def list_groups(self) -> List[GroupInfo]:
        ...
        return [GroupInfo(name=n, members=list(m))
                for n, m in sorted(_GROUP_REGISTRY.items())]

    # ── DENY (complemento de GRANT/REVOKE) ────────────────────────────────────

    def deny(self, privilege: str, object_type: str,
             object_ref: str, principal: str) -> None:
        ...
        self._grants.append(entry)
        print(f"[Unity] DENY {privilege} ON {object_type} "
              f"{object_ref} TO {principal}")
    ...
    def track_lineage(self, source: str, target: str,
                      lineage_type: str = "TABLE") -> None:
    ...
    def get_lineage(self, object_ref: Optional[str] = None) -> List[LineageInfo]:
        ...
        return [LineageInfo(source=l["source"], target=l["target"],
                            lineage_type=l["lineage_type"])
                for l in results]

    def lineage_as_dataframe(self):
        """Devuelve el linaje como DataFrame (simula system.lineage)."""
        rows = [(l["source"], l["target"], l["lineage_type"],
                 l.get("tracked_at", ""))
                for l in _LINEAGE_LOG]
        schema_str = ("source_table STRING, target_table STRING, "
...
class _InformationSchema:
    ...
    def catalogs(self):
        ...
        return self._spark.createDataFrame(
            rows, "catalog_name STRING, comment STRING"
        )
    ...
    def tables(self, catalog_name: Optional[str] = None,
               schema_name: Optional[str] = None):
        tables = self._uc.list_tables(catalog_name, schema_name)
        rows = [(t.catalog_name, t.schema_name, t.name, t.table_type)
                for t in tables]
        return self._spark.createDataFrame(
            rows,
    ...
    def columns(self, catalog_name: Optional[str] = None,
                schema_name: Optional[str] = None,
        ...
        if table_name:
            try:
                ...
                rows = [(cat, sch, table_name, c.name, c.data_type,
                         c.nullable, c.comment) for c in cols]
            except Exception:
                rows = []
        else:
            rows = []
        schema_str = ("table_catalog STRING, table_schema STRING, "
    ...
    def volumes(self, catalog_name: Optional[str] = None,
                schema_name: Optional[str] = None):
    ...
    def table_privileges(self, catalog_name: Optional[str] = None):
        ...
        if catalog_name:
            grants = [g for g in grants
                      if g.object_key.startswith(catalog_name + ".")]
        rows = [(g.principal, g.privilege, g.object_type, g.object_key)
                for g in grants]
        schema_str = ("grantee STRING, privilege_type STRING, "
                      "object_type STRING, object_name STRING")
        return (self._spark.createDataFrame(rows, schema_str)
                if rows else
                self._spark.createDataFrame([], schema_str))

    def routines(self, catalog_name: Optional[str] = None,
                 schema_name: Optional[str] = None):
        """Lista funciones — simula system.information_schema.routines."""
        funcs = self._uc.list_functions(catalog_name, schema_name)
        rows = [(f.catalog_name, f.schema_name, f.name, f.description)
                for f in funcs]
        schema_str = ("routine_catalog STRING, routine_schema STRING, "
